...
TocsiScanner = Scanner(signatures=signaturesData, rootPath=app.root_path)


def startSystemWatcher(thread_resume):
    thread_resume.set()
    systemWatcher(TocsiScanner,SYSTEM_DRIVE,thread_resume)
...
def scans():
    ...
    scanReport = TocsiScanner.scanFolders(location=SCAN_LOCATIONS)
    return scanReport
...
def quarFile():
    ...
    TocsiScanner.quar.quarantine(originalPath, detectionSpace)
    return "Done!"
...
def restoreFile():
    ...
    thread_resume.clear()
    TocsiScanner.quar.restore(originalPath)
...
def removeFile():
    ...
    TocsiScanner.quar.remove(originalPath)
    return "Done"
...
def addFirewallRules(url):
    ...
    try:
        ...
        ips = response.split("\n")
        rule = "netsh advfirewall firewall delete rule name='Tocsi_AV_IP_RULE'"
        ...
        for ip in ips:
            if ip and ip[0] != '!' and "#" not in ip:
                try:
                    ...
                    rule = "netsh advfirewall firewall add rule name='Tocsi_AV_IP_RULE' Dir=Out Action=Block RemoteIP="+ip.rstrip()
                    # print(rule)
